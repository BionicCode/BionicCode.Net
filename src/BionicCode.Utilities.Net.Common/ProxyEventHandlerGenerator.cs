namespace BionicCode.Utilities.Net
{
    ...
    internal static class ProxyEventHandlerGenerator
    {
        ...
        public static Delegate Generate<TEventSource>(string eventName, object target, string targetDelegateMethodName, ParameterList targetDelegateMethodParameterList)
        {
            ...
            SymbolReflectionInfoCacheKey symbolCacheKey = SymbolReflectionInfoCacheKey.CreateForAnonymousMethod(targetType.TypeHandle, targetDelegateMethodName, targetDelegateMethodParameterList, TypeList.Empty, SymbolKind.MemberMethod);
            MethodData proxyDelegateMethodData = SymbolReflectionInfoCache.GetOrCreateMethodDataCacheEntry(ref symbolCacheKey);

            symbolCacheKey = SymbolReflectionInfoCacheKey.CreateForAnonymousEvent(typeof(TEventSource).TypeHandle, eventName, SymbolKind.MemberEvent, isExplicitInterfaceImplementation: false);
        ...
        private static Delegate GenerateProxy(ParameterList eventHandlerParameters, TypeData eventDelegateTypeData, object delegateMethodTarget, MethodData delegateMethodData)
        {
            ...
            {
                ParameterExpression expressionParameter = Expression.Parameter(parameter.ParameterTypeData.Type, parameter.Name);
            ...
            if (delegateMethodData.Parameters.Any())
            {
                ...
                if (lastParameter.IsParams)
                {
                    IEnumerable<Expression> paramsParameterArguments = expressionParameters.Skip(lastParameter.GetParameterInfo().Position)
                                  .Select(parameter => Expression.TypeAs(parameter, lastParameter.ParameterTypeData.Type.GetElementType()))
                                  .Cast<Expression>();
            ...
            Type eventDelegateType = eventDelegateTypeData.Type;
