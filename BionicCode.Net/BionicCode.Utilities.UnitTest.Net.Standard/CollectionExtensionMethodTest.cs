namespace BionicCode.Utilities.UnitTest.Net
{
  ...
  public class CollectionExtensionMethodTest : IClassFixture<TestContext>, IDisposable
  {
    ...
    public CollectionExtensionMethodTest(TestContext context)
    {
    ...
    {
      this.Context.EmptyItems.IsEmpty().Should().BeTrue();
    }

    [Fact]
    public void IsEmpty_ReturnsFalseForNonEmptyCollection()
    {
      this.Context.Items.IsEmpty().Should().BeFalse();
    }
    ...
    [InlineData(20, 4)]
    [InlineData(40, 4)]
    public void TakeRange_Returns4Items(int startIndex, int count)
    {
      this.Context.Items.TakeRange(startIndex, count).Should().HaveCount(4);
    }

    [Theory]
    [InlineData(2, 2)]
    [InlineData(998, 4)]
    public void TakeRange_Returns2Items(int startIndex, int count)
    {
      this.Context.Items.TakeRange(startIndex, count).Should().HaveCount(2);
    }
    ...
    [Fact]
    public void LastOrDefaultInSorted_ReturnsDefaultValueOnFail()
    {
      this.Context.Items.LastOrDefaultInSorted(this.Context.FailingContainsPredicate).Should().Be(default, "the predicate has failed to produce a result.");
    }

    [Fact]
    public void LastOrDefaultInSorted_NotThrowExceptionOnFail()
    {
      this.Context.Items.Invoking(items => items.LastOrDefaultInSorted(this.Context.FailingContainsPredicate)).Should().NotThrow("the predicate has failed to produce a result and returns a default instead of throwing.");
    }

    [Fact]
    public void LastOrDefaultInSorted_Returns_4_OnSuccess()
    {
      this.Context.Items.LastOrDefaultInSorted(this.Context.SuceedingContainsPredicate).Should().Be(4, "the predicate has produced a result.");
    }
    ...
    [Fact]
    public void LastInSorted_ThrowExceptionOnFail()
    {
      this.Context.Items.Invoking(items => items.LastInSorted(this.Context.FailingContainsPredicate)).Should().ThrowExactly<InvalidOperationException>("the predicate has failed to produce a result and in this case must throw.");
    }

    [Fact]
    public void LastInSorted_Returns_4_OnSuccess()
    {
      this.Context.Items.LastInSorted(this.Context.SuceedingContainsPredicate).Should().Be(4, "the predicate has produced a result.");
    }
