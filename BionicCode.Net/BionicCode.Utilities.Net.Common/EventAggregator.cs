...
/// <inheritdoc />
[Obsolete("Please use the WeakEventAggregator and the related interfaces IWeakEventAggregator, IWeakEventAggregatorListenerService and IWeakEventAggregatorPublisherService instead (same namespace)! This type offers a cleaned-up API and several relevant performance and feature improvements and is completely weak event based to allow listeners to be garbage collected without unsubscribing from the observed events.")]
public class EventAggregator : IEventAggregator
{
    ...
    {
        EventHandlerTable = new ConcurrentDictionary<string, List<Delegate>>();
        EventHandlerSynchronizationContextTable = new ConcurrentDictionary<Delegate, SynchronizationContext>();
        EventPublisherTable = new ConditionalWeakTable<object, List<(EventInfo EventInfo, Delegate Handler)>>();
    }
    ...
    public bool TryRegisterObservable(object eventSource, IEnumerable<string> eventNames)
    {
        ...
        foreach (string eventName in eventNames.Distinct())
        {
            ...
            List<(EventInfo EventInfo, Delegate Handler)> publishers = EventPublisherTable.GetOrCreateValue(eventSource);
    ...
    public bool TryRemoveObservable(Type eventSourceType, IEnumerable<string> eventNames, bool removeEventObservers = false)
    {
        ...
        var entries = (IEnumerable<KeyValuePair<object, List<(EventInfo, Delegate)>>>)EventPublisherTable.Where(entry => entry.Key.GetType() == eventSourceType);
        ...
        foreach (KeyValuePair<object, List<(EventInfo, Delegate)>> entry in entries)
        {
            ...
            if (publisherHandlerInfos.Count == 0)
            {
                _ = EventPublisherTable.Remove(eventSource);
            }
        }
    ...
    {
        bool hasRemovedObservable = false;
        if (!EventPublisherTable.TryGetValue(eventSource, out List<(EventInfo EventInfo, Delegate Handler)> publisherHandlerInfos))
        {
            return false;
        }
    ...
    /// <inheritdoc />
    public bool TryRemoveObservable(object eventSource, bool removeEventObservers = false)
    {
        bool hasRemovedObservable = false;

        if (EventPublisherTable.TryGetValue(eventSource, out List<(EventInfo EventInfo, Delegate Handler)> handlerInfo))
        {
            _ = EventPublisherTable.Remove(eventSource);
    ...
    public bool TryRemoveObserver(string eventName, Type eventSourceType, Delegate eventHandler)
    {
        ...
        _ = EventHandlerSynchronizationContextTable.TryRemove(eventHandler, out _);
        return EventHandlerTable.TryRemove(fullyQualifiedEventIdOfSpecificSource, out _);
    }
    ...
    public bool TryRemoveGlobalObserver(string eventName, Delegate eventHandler)
    {
        ...
        _ = EventHandlerSynchronizationContextTable.TryRemove(eventHandler, out _);
        return EventHandlerTable.TryRemove(fullyQualifiedEventIdOfGlobalSource, out _);
    }
    ...
    public bool TryRemoveAllObservers(string eventName, Type eventSourceType)
    {
        string fullyQualifiedEventIdOfSpecificSource =
          CreateFullyQualifiedEventIdOfSpecificSource(eventSourceType, eventName);
        if (EventHandlerTable.TryRemove(fullyQualifiedEventIdOfSpecificSource,
    ...
    public bool TryRemoveAllObservers(Type eventSourceType)
    {
        ...
        for (int index = EventHandlerTable.Count - 1; index >= 0; index--)
        {
            KeyValuePair<string, List<Delegate>> handlersEntry = EventHandlerTable.ElementAt(index);
            if (handlersEntry.Key.StartsWith(fullyQualifiedEventIdOfSpecificSourcePrefix, StringComparison.Ordinal))
            {
                result |= EventHandlerTable.TryRemove(handlersEntry.Key, out _);
    ...
    {
        if (EventHandlerTable.TryGetValue(fullyQualifiedEventName, out List<Delegate> handlers))
        {
            handlers.Add(eventHandler);
            return true;
        }

        return EventHandlerTable.TryAdd(fullyQualifiedEventName, new List<Delegate>() { eventHandler });
    }
    ...
    {
        _ = EventHandlerSynchronizationContextTable.TryAdd(eventHandler, synchronizationContext);
        if (EventHandlerTable.TryGetValue(fullyQualifiedEventName, out List<Delegate> handlers))
        {
            handlers.Add(eventHandler);
            return true;
    ...
    private void DelegateHandleEvent((Type EventHandlerType, ICollection<string> EventIds) eventInfo, object sender, object args)
    {
        IEnumerable<Delegate> handlers = eventInfo.EventIds
          .SelectMany(
            eventId => EventHandlerTable.TryGetValue(eventId, out List<Delegate> delegates)
              ? delegates
              : new List<Delegate>());
        ...
        foreach (Delegate handler in handlers)
        {
            try
            {
                if (EventHandlerSynchronizationContextTable.TryGetValue(handler, out SynchronizationContext synchronizationContext))
                {
    ...
    private string? FormatTypeName(Type typeToFormat, bool isFullyQualified = true)
    {
        ...
        return originalTypeName[..originalTypeName.IndexOf("`", StringComparison.OrdinalIgnoreCase)] + "<" + string.Join(", ", genericArguments.Select(type => FormatTypeName(type))) + ">";
    }
