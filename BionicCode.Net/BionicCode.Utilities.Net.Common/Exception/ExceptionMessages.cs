...
namespace BionicCode.Utilities.Net
{
  ...
  internal class ExceptionMessages
  {
    ...
    public static string GetMissingCreationMemberOfProfiledTypeExceptionMessage(TypeData profiledType) => $@"Unable to create an instance of the type {profiledType.FullyQualifiedDisplayName} that declares the profiled member.{System.Environment.NewLine}The profiled/declaring type must define a parameterless constructor. Alternatively, provide a factory (a class member that provides an initialized instance) that satisfies all of the following conditions: it must return a properly initialized instance of the declaring owner type or a delegate that returns this instance, and that member must be a static field, property or method or a non-static constructor. By convention, the first member that satisfies those conditions is taken in the following order: method, property, constructor, field. To override this convention decorate a member that satisfies the aforementioned conditions with the '{nameof(ProfilerFactoryAttribute)}' and provide the argument list to the attribute's constructor if required. The access modifier (e.g. private, internal, public) can be of any symbolAttributes and is not relevant, this means even private members are selected as instance provider.";
    public static string GetArgumentListMismatchExceptionMessage() => $"The argument list privided by the '{nameof(ProfilerMethodArgumentAttribute)}' does not match the signature of the profiled member. The argument list must define the arguments in the same order and of the same type as the parameter list of the profiled member.";
